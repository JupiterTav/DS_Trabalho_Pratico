...
class Cabecalho(ctk.CTkFrame):
    def __init__(self, master: ctk.CTk, text_area: ScrollableCampoTexto) -> None:
        ...
        self.upload_txt: ctk.CTkButton = ctk.CTkButton(self, width=30, height=30, text="Carregar texto", corner_radius=70, font=("Arial", 16, "bold"), 
                                                       command=lambda: self.upload_arq_texto(text_area))
        self.upload_txt.grid(row=0, column=0, sticky="w", padx=(0, 10))

        self.download_txt: ctk.CTkButton = ctk.CTkButton(self, width=30, height=30, text="Salvar texto", corner_radius=70, font=("Aria", 16, "bold"),
