...
class ScrollableCampoTexto(ctk.CTkScrollableFrame):
    ...
    def adiciona_campo(self) -> CampoTextoEditavel:
        ...
        camp_edit: CampoTextoEditavel = CampoTextoEditavel(self,
                                                           oitava_inicial=self.__oitavas_padrao[
                                                               self.__ciclo_valores_iniciais],
                                                           volume_inicial=self.__volumes_padrao[
                                                               self.__ciclo_valores_iniciais],
                                                           instrumento_inicial=self.__instrumentos_padrao[
                                                               self.__ciclo_valores_iniciais])
        camp_edit.grid(row=self._quant_campos, column=0, pady=15)
    ...
    def __desenha_add_button(self):
        add_button = ctk.CTkButton(self, width=30, height=30, corner_radius=20, text="+", font=("Arial", 18),
                                   command=lambda: self.adiciona_campo())
